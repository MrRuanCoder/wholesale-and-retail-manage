  ...
  const search = () => {
    service
      ...
      .then(({ data: res }) => {
        if (res.code === 20000) {
          setList(
            res.data.filter((item) => item.goodsName.includes(filteredName))
          );
        } else throw new Error(res.message);
      })
      .catch((e) => {
      ...
      <TableContainer>
        <Table sx={{ minWidth: 650 }} size="small">
          <TableHead>
            ...
            <TableRow>
              {rows[0]?.numbers.map((item) => (
                <TableCell align="center">{item.storageName}</TableCell>
              ))}
            </TableRow>
          </TableHead>
          <TableBody>
            {rows.map((row) => (
              ...
              >
                ...
                <TableCell align="center">
                  {row.numbers.reduce((prev, cur) => prev + cur.number, 0) *
                    row.purchasePrice}
                </TableCell>
              </TableRow>
            ))}
